...
class EditDelta:
    ...
    entities_preserved_correctly: float = 0.0
    intended_changes_applied: float = 0.0
    unintended_changes: float = 0.0
...
class ChangeDetector:
    ...
    def compute_delta(self, before: SceneAnalysis, after: SceneAnalysis) -> EditDelta:
        """
        Compute the delta between two scene analyses.
        
        This method:
        1. Takes the before and after SceneAnalysis objects as input
        2. Matches entities between the before and after scenes by spatial overlap (IoU > 0.5)
        3. For each matched pair of entities:
           - If after entity is None → add to removed entities
           - If entities are similar (by similarity metrics) → add to preserved entities
           - Otherwise → add to modified entities
...
if __name__ == "__main__":
    # Create mock entities for before scene
    before_entities = [
        Entity(id="sky_1", name="sky", bbox=(0, 0, 500, 200), 
               color=(135, 206, 235), centroid=(250, 100)),
        Entity(id="tree_1", name="tree", bbox=(100, 200, 200, 400),
               color=(34, 139, 34), centroid=(150, 300)),
        Entity(id="mountain_1", name="mountain", bbox=(300, 200, 500, 400),
               color=(105, 105, 105), centroid=(400, 300))
    ]
    
    # Create mock entities for after scene (some changed)
    after_entities = [
        Entity(id="sky_1", name="sky", bbox=(0, 0, 500, 200),
               color=(255, 165, 0), centroid=(250, 100)),  # Changed: more dramatic orange color
        Entity(id="tree_1", name="tree", bbox=(100, 200, 200, 400),
               color=(34, 139, 34), centroid=(150, 300)),  # Preserved
        Entity(id="mountain_1", name="mountain", bbox=(300, 200, 500, 400),
               color=(169, 169, 169), centroid=(400, 300))  # Changed: lighter gray
    ]
    
    # Create scene analyses
    before_scene = SceneAnalysis(entities=before_entities, image_size=(500, 400))
    after_scene = SceneAnalysis(entities=after_entities, image_size=(500, 400))
    ...
    print(f"Entities Preserved Correctly: {delta.entities_preserved_correctly:.2f}")
    print(f"Intended Changes Applied: {delta.intended_changes_applied:.2f}")
    print(f"Unintended Changes: {delta.unintended_changes:.2f}")